...
def _serialize(r: GemRadarAlert) -> dict:
    return {
        ...
        "price_change_5m": float(r.price_change_pct) if r.price_change_pct is not None else None,
        "price_change_1h": float(r.price_change_1h_pct) if r.price_change_1h_pct is not None else None,
        "price_change_24h": float(r.price_change_24h_pct) if r.price_change_24h_pct is not None else None,
        "market_cap_usd": float(r.market_cap_usd) if r.market_cap_usd is not None else None,
        "liquidity_usd": float(r.liquidity_usd) if r.liquidity_usd is not None else None,
