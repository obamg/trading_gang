...
async def status(_user: CurrentUser):
    ...
    return {
        ...
        "config": {
            ...
            "max_open_risk_pct": float(app_settings.bot_max_open_risk_pct),
            "loss_throttle_stops": int(app_settings.bot_loss_throttle_stops),
            "loss_throttle_factor": float(app_settings.bot_loss_throttle_factor),
            "max_turnover_notional_pct": float(app_settings.bot_max_turnover_notional_pct),
...
def _serialize_trade(t: BotTrade) -> dict:
    return {
        ...
        "initial_stop_price": float(t.initial_stop_price) if t.initial_stop_price is not None else None,
        "peak_price": float(t.peak_price) if t.peak_price is not None else None,
        "partial_exit_price": float(t.partial_exit_price) if t.partial_exit_price is not None else None,
        "partial_exit_at": t.partial_exit_at.isoformat() if t.partial_exit_at else None,
        "partial_pnl_usd": float(t.partial_pnl_usd) if t.partial_pnl_usd is not None else None,
        "partial_qty": float(t.partial_qty) if t.partial_qty is not None else None,
        "status": t.status,
    }
